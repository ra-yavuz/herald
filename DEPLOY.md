# Deploying a new release of herald
...
1. **Bump the version**:
   - `debian/changelog` &rarr; new entry `herald (X.Y.Z-1) unstable; urgency=low` with notes
...
- **lint** + **build-deb** &rarr; produces `dist/herald_X.Y.Z-1_all.deb`
...
Within ~5 min total, `sudo apt update && sudo apt install herald` serves the new version.

## What if the tag run fails?

Watch CI at https://github.com/ra-yavuz/herald/actions. Common causes:
...
|---|---|---|
| GitHub Release | `https://github.com/ra-yavuz/herald/releases/tag/vX.Y.Z` | `release` job |
| .deb in apt repo | `https://ra-yavuz.github.io/apt/pool/main/h/herald/` | `add-package` &rarr; `publish` |
| Apt index | `https://ra-yavuz.github.io/apt/dists/stable/main/binary-amd64/Packages` | `publish` |
| Project Pages | `https://ra-yavuz.github.io/herald/` | redeploys on any push to `main` |

## Same flow lives in hydra-llm, inhibit-charge, meowtrics
